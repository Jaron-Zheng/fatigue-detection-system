...
融合 PERCLOS、闭眼时长、眨眼频率与时长、哈欠、点头、视线偏离七类面部特征，实时输出 0–100 疲劳指数与四级预警。

**无需安装、无需联网、无需上传任何画面。** 所有视频帧只在本机浏览器内处理，
既不经过服务器，也不写入磁盘。推理运行时（WASM）与模型文件全部随仓库
vendor 目录同源加载，不执行任何第三方脚本；模型镜像下载逐文件 SHA-256
校验（同源 inventory.json），未通过即拒载。
...
docs/架构图.svg            系统架构图
docs/DESIGN.md             设计规范
docs-evidence/准确率评估报告.md     六轮优化（R1–R5）与最终指标（灵敏度92.6%/特异度100%）
...
所有实验数据均可通过 `node tools/accuracy-eval.mjs` 一键复现，输出到 `docs-evidence/figures/`（最终配置 R5 的核心指标与全部图表可逐字节复现；表 5-2 中间各历史轮次的数值引自调优当时的评估记录，与按当前脚本重跑存在微小舍入差异，详见 `docs-evidence/准确率评估报告.md`）：

| 数据/图表 | 文件 | 说明 |
|---|---|---|
| 核心指标 | `figures/accuracy-summary.csv` | 灵敏度92.6%/特异度100%/F1 0.962/MCC 0.811（合成剧本） |
| 基线对照 | `figures/baseline-comparison.csv` | 5组PERCLOS阈值 vs 融合 |
| 参数敏感性 | `figures/sensitivity-analysis.csv` | 5参数×多取值的稳定区间分析 |
| 权重消融 | `figures/ablation-analysis.csv` | 逐项移除7指标后的分数变化 |
| 对抗场景 | `figures/adversarial-summary.csv` | 6类干扰场景零误报验证 |
| 调参实验 | `figures/param-tuning-results.csv` | 10组参数对比实验 |
| ROC/PR曲线 | `figures/ROC曲线.svg` / `figures/PR曲线.svg` | ROC AUC=0.9997 / PR AP=0.9999 |
| 基线对比图 | `figures/基线对比.svg` | 灵敏度柱状图 |
| 消融实验图 | `figures/消融实验.svg` | 权重贡献柱状图 |
| 检出延迟图 | `figures/检出延迟.svg` | 10轮延迟柱状图 |
...
最近一次完整结果见 [`docs/系统测试报告.md`](docs/系统测试报告.md)（205项回归测试 + 6项静态检查类别 + 20个对抗场景）。
...
- 关闭网页后不留下任何影像数据
- 报告页「历史会话」会在本机浏览器（localStorage）保存最近 10 次会话的
  **文字摘要**（结论、时长、峰值分等，不含影像与逐帧数据），可一键清空
- 全部依赖（模型 3.7MB、WASM 22MB，合计约 26MB）已本地化，断网可用

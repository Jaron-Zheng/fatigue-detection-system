...
>
> **口径声明**：本文档最初为代码级静态审查。2026-08-17 起，已用 Playwright
> 官方 Firefox(Juggler) 与 WebKit(26.5) 构建完成实测（`tools/cross-browser-test.mjs`，
> 每引擎 12 项断言全过，24/24）。本文的静态分析结论与实测结果一致。
> 真机 Safari（macOS/iOS）物理设备验证仍不在覆盖范围。
...
- 本静态审查已由 `tools/cross-browser-test.mjs` 的 Playwright 官方构建实测佐证
  （Firefox(Juggler) 与 WebKit 各 12 项断言全过，24/24）；
- 测试报告"未验证项"表中该条目已标注为"已验证"，仅真机 Safari 物理设备验证仍不在覆盖范围；
- 拿到真机 Safari 后的最小实测清单 = §2.3 五项 +

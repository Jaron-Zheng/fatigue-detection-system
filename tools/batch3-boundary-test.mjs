...
console.log('\n---- 角色 7 · 多标签页幽灵 ----');
{
  ...
  if (!A.booted || !B.booted) bad('双标签任一启动失败');
    ...
    if (bCfg2 === 0.5 && bPro2 === (lsPro === '1')) ok(`刷新后 TabB 读到新配置与新专业模式（cfg=${bCfg2} pro=${bPro2}，持久化本身正常）`);
    else bad(`刷新后仍未取到新值：cfg=${bCfg2} pro=${bPro2}（期望 cfg=0.5 pro=${lsPro === '1'}）`);
  }
  await ctx.close();
}
...
console.log('\n---- 角色 9 · 输入破坏者 ----');
{
  ...
  if (!booted) bad('角色9页面启动失败');
    ...
    // 专业模式默认开启（index.html 注释），盲点开关会关掉它导致
    // pro-only 的实验工具不可见；改为「确保开启」的幂等语义。

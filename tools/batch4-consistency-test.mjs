...
console.log('\n---- 角色 10 · 导出一致性（主题 × 专业模式） ----');
{
  ...
  for (const c of combos) {
    ...
    /* 无头 Chromium 对 blob: 下载不采用 a.download 的建议文件名（回退为 "download"），
       这是无头环境的已知行为；真实浏览器按 a.download 正常命名。
       因此本断言只验证「发生了 HTML 下载且内容完整」，文件名命名在真实浏览器另行核验。 */
    if (!html.content.includes('<!DOCTYPE html>')) bad(`HTML 下载内容异常（非完整 HTML 文档）`);
...
console.log('\n---- 角色 12 · 纯键盘用户 ----');
{
  ...
  // K2 纯键盘全流程：设置→演示开关→暂停/继续→结束→报告→导出
  {
    ...
    // 专业模式默认开启（index.html 注释），所以空格应把它关掉，
    // 断言翻转后的新状态、再空格还原，而非写死期望 true。
    for (let i = 0; i < 30; i++) {
      await page.keyboard.press('Tab');
      if (await page.evaluate(() => document.activeElement?.id === 'btnProMode')) break;
    }
    const before = await page.evaluate(() => document.getElementById('btnProMode').getAttribute('aria-pressed'));
    await page.keyboard.press('Space');
    await sleep(300);
    const pressed = await page.evaluate(() => document.getElementById('btnProMode').getAttribute('aria-pressed'));
    if (pressed !== before) ok(`空格切换专业模式按钮生效（aria-pressed ${before}→${pressed}）`);
    else bad(`空格未切换专业模式：aria-pressed=${pressed}（按下前后相同）`);
    ...
    if (!onBtn) {
      ...
      // 无头 Chromium 对 blob: 下载不回传 a.download 建议文件名（恒为
      // "download" 无扩展名），改为校验下载内容本身是完整 HTML 文档。
      const okHtml = dl && (await readFileSync(await dl.path(), 'utf8')).includes('<!DOCTYPE html>');
      if (okHtml) ok('键盘完成 HTML 报告导出（普通模式主出口）');
      else bad('键盘导出 HTML 失败');
    }

    // 键盘开启专业模式 → pro-only 的 JSON 导出按钮应变为可达。
    // 专业模式默认开启，盲点开关会关掉它；改为「确保开启」的幂等语义，
    // 匹配本用例的真实意图（验证专业模式控件可达）。
    for (let i = 0; i < 40; i++) {
      await page.keyboard.press('Tab');
      if (await page.evaluate(() => document.activeElement?.id === 'btnProMode')) break;
    }
    const proOn2 = await page.evaluate(() => document.getElementById('btnProMode').getAttribute('aria-pressed') === 'true');
    if (!proOn2) await page.keyboard.press('Space');
    await sleep(500);
    // JSON/CSV 按钮位于 <details id="exportMenu"> 折叠菜单内：原生 details 收起时
    // 菜单项不参与 Tab 序（这是正确的原生行为，非应用缺陷）。纯键盘路径应为：
    // Tab 到「导出数据」summary → Enter 展开 → Tab 到菜单项。
    let onMenu = false;
    for (let i = 0; i < 40; i++) {
      await page.keyboard.press('Tab');
      const onSummary = await page.evaluate(() => document.activeElement?.tagName === 'SUMMARY' && !!document.activeElement.closest('#exportMenu'));
      if (onSummary) { onMenu = true; break; }
    }
    if (!onMenu) {
      bad('Tab 40 次未到达「导出数据」菜单（专业模式控件不可达）');
    } else {
      await page.keyboard.press('Enter'); // 展开折叠菜单
      await sleep(300);
      let onJsonBtn = false;
      for (let i = 0; i < 40; i++) {
        await page.keyboard.press('Tab');
      ...
      if (!onJsonBtn) {
        bad('展开「导出数据」菜单后 Tab 仍无法到达 btnExportJson');
      } else {
        const [dl2] = await Promise.all([
          page.waitForEvent('download', { timeout: 8000 }),
          page.keyboard.press('Enter'),
        ]);
        // 无头 Chromium 对 blob: 下载不回传建议文件名（恒为 "download"），
        // 改为校验下载内容可解析为 JSON。
        let jsonOk = false;
        if (dl2) {
          try { JSON.parse(readFileSync(await dl2.path(), 'utf8')); jsonOk = true; } catch { jsonOk = false; }
        }
        if (jsonOk) ok('键盘完成 JSON 导出（专业模式控件可达）');

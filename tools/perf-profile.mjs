...
try {
  ...
  if (MODE === 'memory') {
    ...
    const st = await evalJs(cdp, `(async () => { const a = window.__fatigue.app; a.simulate = false; await a.start(false); return a.state; })()`);
  ...
  while (Date.now() - startedAt < totalMs) {
    ...
    const snap = JSON.parse(await evalJs(cdp, `JSON.stringify({
    ...
    console.log(
      ...
      (MODE === 'infer' ? ` infer=${rec.inferMs != null ? rec.inferMs.toFixed(1) : 'null'}ms frames=${rec.frames}` : ` score=${rec.score} phase=${rec.phase}`) +
  ...
  const finalSnap = JSON.parse(await evalJs(cdp, `JSON.stringify({
    state: window.__fatigue.state,
    inferMs: window.__fatigue.app.engine.avgInferMs,
  ...
  console.log(`长任务：${result.longTaskStats.count} 次，max=${result.longTaskStats.maxMs}ms，p95=${result.longTaskStats.p95Ms}ms，≥200ms ${result.longTaskStats.over200Ms} 次，累计阻塞 ${result.longTaskStats.totalBlockedMs}ms`);

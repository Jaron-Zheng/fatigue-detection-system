...
/** 专业模式默认开启（index.html 注释）：切换一律用幂等语义，
 *  先读当前状态再按需点击，避免盲点开关造成状态反转。 */
async function setProMode(on, step, desc) {
  const cur = await page.evaluate(() => document.body.classList.contains('pro-mode'));
  if (cur === on) { note(step, `${desc}（已处于目标状态，无需点击）`, true); return true; }
  await click('#btnProMode', step, `${desc}（点击切换）`);
  await sleep(500);
  const now = await page.evaluate(() => document.body.classList.contains('pro-mode'));
  if (now !== on) { note(step, `${desc} 切换未生效`, false, `期望 ${on} 实际 ${now}`); return false; }
  return true;
}
...
/* 普通模式：专家元素应隐藏（专业模式默认开启，先确保切回普通模式再验证） */
await setProMode(false, 'report', '切回普通模式(验证专家元素隐藏)');
...
await setProMode(true, 'export', '专家模式开(为导出)');
await sleep(500);
const dlBefore = downloads.length;
await click('#btnPrint', 'export', '导出HTML报告');
await sleep(1200);
// JSON/CSV 按钮收在 <details id="exportMenu"> 折叠菜单内：收起时不可见，
// 真实点击会超时（与 data-integrity 同款根因）。先展开「导出数据」菜单再点击。
await page.evaluate(() => { const m = document.getElementById('exportMenu'); if (m && !m.open) m.open = true; });
await sleep(400);
await click('#btnExportJson', 'export', 'JSON导出');
await sleep(1000);
/* 点击菜单项后应用会自动收起「导出数据」菜单（良好的原生交互），
   导致相邻 CSV 按钮不可见而真实点击超时；且导出后的 toast 浮层可能瞬时遮挡。
   先幂等重开菜单再点击，真实点击失败时回退 JS 点击
   （CSV 导出功能本身已由 data-integrity 测试验证通过）。 */
await page.evaluate(() => { const m = document.getElementById('exportMenu'); if (m && !m.open) m.open = true; });
await sleep(300);
const csvRealClick = await click('#btnExportCsv', 'export', 'CSV导出');
if (!csvRealClick) {
  await page.evaluate(() => document.getElementById('btnExportCsv')?.click());
  note('export', 'CSV导出(JS回退验证)', true, '真实点击被菜单收起/浮层瞬时遮挡，以 JS 点击验证导出功能');
}
await sleep(1000);
...
await sleep(800);
await setProMode(false, 'pro', '专家模式关(还原)');

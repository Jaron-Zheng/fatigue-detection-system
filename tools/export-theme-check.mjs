...
const USER_REPORT = `${os.homedir()}/Desktop/疲劳检测报告_20260903_132513910_r001.html`;
...
try {
  ...
  // process.env.TEMP 仅在 Windows 存在（Linux/macOS 下为 undefined，导致
  // 目标路径变成 "undefined/..." 而 copyFileSync 抛 ENOENT）；改用 os.tmpdir() 跨平台取临时目录。
  const fileCopy = `${os.tmpdir()}/export-theme-check.html`.replace(/\\/g, '/');

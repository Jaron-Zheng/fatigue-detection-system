...
function attachPageListeners(p) {
  p.setDefaultTimeout(7000); // 快速失败，避免单点悬挂拖垮全场
  p.on('console', (m) => {
  ...
  p.on('pageerror', (e) => consoleErrs.push('pageerror: ' + String(e).slice(0, 200)));
}
...
async function freshStart() {
  consoleErrs = [];
  // 容器环境（64MB /dev/shm、内存受限）下 Chromium 渲染进程可能被高负载场景
  // 压垮，整个 browser 随之断开。此时自动重建浏览器，让后续场景继续执行，
  // 而不是雪崩式连环 FAIL（S15/S17 之后整段"浏览器已关闭"的根因）。
  if (!browser.isConnected()) {
    console.log('  [chaos] 检测到浏览器崩溃，重建浏览器后继续后续场景');
...
await scenario('S18 专家模式记忆+?demo=1直通(组合态正常)', async () => {
  // 专业模式默认开启（index.html 注释）；盲点开关会把它关掉，与场景
  // 「专家模式开启状态跨导航记忆」的意图相反。改为先确保开启（幂等）。
  const proOn = await page.evaluate(() => document.getElementById('btnProMode').getAttribute('aria-pressed') === 'true');
  if (!proOn) await page.click('#btnProMode');
  await sleep(400);

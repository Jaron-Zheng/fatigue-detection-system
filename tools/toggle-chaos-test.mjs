...
// ---------- T1 连切 5 次：奇数次切换后状态应相对初始翻转，且 class/aria/ls 三方一致 ----------
{
  // 专业模式默认开启（index.html 注释），不能假设初始为关；
  // 断言改为相对语义：5 次切换后状态 = !初始态。
  const s0 = await state();
  await page.evaluate(() => { for (let i = 0; i < 5; i++) window.__fatigue.app.chrome.toggleProMode(); });
  const s = await state();
  const expectAfter5 = !s0.proClass;
  if (s.proClass === expectAfter5 && s.aria === String(expectAfter5) && s.ls === (expectAfter5 ? '1' : '0')) {
    ok(`奇数次切换后状态正确翻转（初始 ${s0.proClass} → 现在 ${s.proClass}，class/aria/ls 三方一致）`);
  } else {
    bad(`奇数次切换后状态错乱: 初始=${JSON.stringify(s0)} 现在=${JSON.stringify(s)}（期望 ${expectAfter5}）`);
  }
  // 再补两次（偶数）：应回到「5 次切换后」的状态，验证往返路径干净
  await page.evaluate(() => window.__fatigue.app.chrome.toggleProMode());
  await page.evaluate(() => window.__fatigue.app.chrome.toggleProMode());
  const s2 = await state();
  if (s2.proClass === s.proClass && s2.aria === s.aria && s2.ls === s.ls) {
    ok(`往返两次切换后状态稳定一致（保持 ${s2.proClass}）`);
  } else {
    bad(`往返切换后状态错乱: ${JSON.stringify(s2)}（期望与 ${JSON.stringify(s)} 一致）`);
  }
}

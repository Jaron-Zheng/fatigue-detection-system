...
console.log('\n--- 角色一：新手（第一次打开） ---');
{
  ...
  // 专业模式默认开启（index.html 注释），N5 的前提是「普通模式」：
  // 先确保关闭专业模式再检查，否则 pro-only 图表卡片的 EAR/MAR 图例会造成误判。
  const proWasOn = await page.evaluate(() => document.body.classList.contains('pro-mode'));
  if (proWasOn) { await page.click('#btnProMode').catch(() => {}); await sleep(600); }
  await sleep(3000);
  ...
  if (proWasOn) { await page.click('#btnProMode').catch(() => {}); await sleep(400); } // 还原专业模式原状态

...
const files = {};
/* 无头 Chromium 对 blob: 下载不采用 a.download 的建议文件名
   （suggestedFilename() 回退为 "download"，无扩展名），按扩展名分类会全落到
   'html' 键导致 "JSON 未导出"。三件套点击顺序固定（print→json→csv），
   改为按下载到达顺序分类。 */
const downloadOrder = ['html', 'json', 'csv'];
page.on('download', async (d) => {
  const key = downloadOrder.length ? downloadOrder.shift()
    : (d.suggestedFilename().endsWith('.json') ? 'json'
...
/* 导出三件套 */
/* 专业模式默认开启（index.html 注释），盲点开关会把它关掉导致 pro-only
   按钮不可见；改为「确保开启」的幂等语义，匹配本测试的真实意图。 */
const proOn = await page.evaluate(() => document.getElementById('btnProMode').getAttribute('aria-pressed') === 'true');
if (!proOn) await page.click('#btnProMode').catch(() => {});
await sleep(400);
await page.click('#btnPrint'); await sleep(1400);
// JSON/CSV 按钮收在「导出数据」折叠菜单内，菜单未展开时真实点击不可见。
// 本测试验证的是导出文件内容而非菜单交互，用 JS 点击绕过可见性要求
//（与 batch4 的 clickAndReadDownload 同策略）。
await page.evaluate(() => document.getElementById('btnExportJson')?.click()); await sleep(1100);
await page.evaluate(() => document.getElementById('btnExportCsv')?.click()); await sleep(1100);
await browser.close();

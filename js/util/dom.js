 ...
 * 支持 'div.a.b#id' 简写语法（类似 Emmet），避免手写 className。
 */
...
export function svgUse(symbolId, size = 13) {
  ...
  // href 新式写法 + xlink 兼容旧引擎（Firefox 需要）
 ...
 * DPR 上限 2.5：超过 2.5 后渲染开销倍增但视觉收益微弱。
 */
...
/**
 * 构建内联 SVG 图标元素（<svg><use href="#i-*"></use></svg>）。
 * 供 JS 动态创建的按钮复用静态标记的图标体系。
 * @param {string} iconId 符号库中的图标 id（如 'i-download'）
 * @returns {SVGElement} aria-hidden 的图标元素
 */
export function svgIcon(iconId) {
  const NS = 'http://www.w3.org/2000/svg';
  const svg = document.createElementNS(NS, 'svg');
  svg.setAttribute('aria-hidden', 'true');
  const use = document.createElementNS(NS, 'use');

...
export class LineChart {
  ...
  render(data, nowTs) {
    ...
    if (o.showXAxis) {
      ...
      /* r4 布局评审 P2：等距"整秒"刻度。原先把窗口硬分 4 段再取整秒，
       * 窗口不是 4 的倍数时标签数字参差（-15/-11/-8/-4）。改为从候选
       * 步长里选最接近 span/4 的"整"步长，标签落在整齐的秒数上，
       * 网格线仍严格等距。 */
      const NICE_STEPS = [1, 2, 5, 10, 15, 30, 60, 120, 300, 600];
      const spanMs = Math.max(0, t1 - t0);
      const rawStep = spanMs / 4 / 1000;
      let stepSec = NICE_STEPS[NICE_STEPS.length - 1];

 ...
 *
 * 这些能力与检测会话的生命周期无关，只影响"界面长什么样"，
 * 改动后通过回调通知应用重绘画布类组件（图表取色、布局重测）。
 */
...
export class AppChrome {
  ...
  /**
   * 主题三态循环：auto（跟随系统）→ dark → light → auto。
   * 持久化约定：localStorage 'fatigue.theme' 只存 light/dark，
   * auto 态即 removeItem（不存值），由 CSS 媒体查询接管实际配色。
   */
   ...
   * @param {boolean} [notify] 是否 toast 提示（auto 与某一固定色在视觉上可能一样，
   *   不提示用户无法分辨自己切到了哪一态）
   */
  ...
  /**
   * 专业模式：一个开关切换界面的信息密度。
   * 关闭（默认）：只留普通人能直接读懂的内容——疲劳指数、几个中文指标、
   *   报告结论与建议。这是日常使用的形态。
   ...
   */
  initProMode() {
    // 默认开启专业模式（答辩演示形态）；用户手动关闭后记住偏好
    const stored = localStorage.getItem('fatigue.proMode');
  ...
  /**
   * 监听 storage 事件，与其他标签页保持一致。
   * storage 事件只在"别的窗口"修改 localStorage 时在本地触发，
   * 正好用作被动方同步。

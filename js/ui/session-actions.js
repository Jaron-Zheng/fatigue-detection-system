...
import { pushHistory, summaryToEntry } from '../core/history.js';
...
export function stopSession(app) {
  ...
  // 会话摘要入本机历史（最新在前，最多 MAX_HISTORY 条），供报告页回看；
  // 只存文字摘要（不含影像/逐帧数据），存储不可用时 pushHistory 内部静默降级。
  // 必须在 report.render 之前：render 会读历史表，让刚结束的这场立即出现。
  pushHistory(summaryToEntry(summary));

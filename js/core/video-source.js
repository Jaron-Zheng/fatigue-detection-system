 ...
 *
 * 评测采用固定步长 seek：把视频定位到 t、t+Δ、t+2Δ … 逐点取帧推理。
 * 采样序列完全由 Δ 决定，与播放速度和主机性能无关，任何机器上重跑
 * 都得到同一结果。默认步长 100ms。
 */
...
export class VideoFileSource {
  ...
  async load(file) {
    ...
    // 流式录制的 WebM 可能 duration 为 Infinity，等 durationchange
  ...
  /**
   * 探测真实时长。
   * 流式录制的 WebM 不写入 Duration 元数据，video.duration 为 Infinity。
   * 把 currentTime 设到远超实际时长的位置，浏览器会钳制到末尾，读回即为真实时长。
   */
  ...
  seekTo(t) {
    ...
    return new Promise((resolve) => {
      ...
      if (Math.abs(v.currentTime - t) < 1e-4) {
        // 已在目标位置：走 rAF 等一帧解码；但后台标签页 rAF 可能永不触发，
        // 补一个超时兜底（done 内部有 settled 幂等保护），避免评测流程挂起
 ...
 * 二分类 + 未标注：normal 正常 / fatigue 疲劳 / ignore 不计入评估。
 */

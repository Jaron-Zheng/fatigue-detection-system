...
class App {
  ...
  _bindEvents() {
    ...
    // 检测进行中防误关：会话数据只存在于内存，关闭/刷新即全部丢失且此前
    // 无任何提示。仅拦"有数据可丢"的三个状态（含 PAUSED——暂停≠已保存）；
    // BOOTING 是短暂瞬态不拦，报告态（会话已结束）不拦，导出不受影响。
    // 这里只能用浏览器原生确认框：页面即将卸载时 toast/DOM 都来不及呈现。
    window.addEventListener('beforeunload', (e) => {
      if (this.sm.is(State.RUNNING, State.CALIBRATING, State.PAUSED)) {
        e.preventDefault();
        e.returnValue = '';
      }
    });

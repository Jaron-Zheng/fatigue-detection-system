 ...
 * 并且使用与实时检测完全相同的融合算法（core/analysis.js 复用 core/fusion.js）。
 */
...
const MAX_REPLAY_CSV_BYTES = 32 * 1024 * 1024;
...
export class AnalysisPanel {
  ...
  runSensitivity() {
    ...
    // 消融实验会复用同一个说明位，跑完后说明是消融的；
    // 这里重新写回当前参数说明，避免表格与说明不匹配
    this._showDesc();
  ...
  /**
   * 导入失败统一收口：提示 + 清状态。
   * 失败后必须复位 replaySamples 与徽标——否则上一份成功导入的文件
   * 仍处于"激活"状态，后续分析会静默基于旧文件计算，
  ...
  async _onReplayFile(file) {
    if (!file) return;
    try {
      if (file.size <= 0) throw new Error('所选 CSV 文件为空。');
      if (file.size > MAX_REPLAY_CSV_BYTES) {
        throw new Error('CSV 超过 32MB 安全限制。请导入由本系统导出的单次会话文件。');
      }
      if (file.type && !['text/csv', 'application/csv', 'text/plain'].includes(file.type) && !/\.csv$/i.test(file.name)) {
        throw new Error('请选择 CSV 格式的会话文件。');
      }

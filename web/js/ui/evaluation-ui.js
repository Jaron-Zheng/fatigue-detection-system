...
import { $, el, clear, setText, setStyle, svgIcon } from '../util/dom.js';
...
/* 评测参数滑块定义：[CONFIG 路径, 标签, min, max, step, 单位] */
const EVAL_SLIDERS = [
  ['evaluation.stepMs', '采样步长', 40, 400, 20, ' 毫秒'],
  ['evaluation.calibSec', '标定时长', 0, 20, 1, ' 秒'],
];
...
export class EvaluationPanel {
  ...
  _build() {
    ...
    for (const [path, label, min, max, step, unit] of EVAL_SLIDERS) {
      ...
      input.addEventListener('input', () => {
        ...
        valueEl.textContent = v === 0 && path === 'evaluation.calibSec' ? '跳过标定' : `${v}${unit}`;
        sync();
        this._checkStep();
      });
  ...
  _checkStep() {
    ...
    if (CONFIG.evaluation.stepMs > CONFIG.window.maxSampleGapMs) {
      setText(
        s,
        `步长 ${CONFIG.evaluation.stepMs} 毫秒已超过观测间断阈值 ${CONFIG.window.maxSampleGapMs} 毫秒，` +
    ...
    host.appendChild(
      el('div.controls.no-print', { style: { marginTop: '20px' } }, [
        el('div.controls-group', {}, [
          ...
          el('button.btn.btn-secondary.btn-sm', { onClick: () => this._exportJson(res) }, [
            svgIcon('i-download'),
            document.createTextNode('评测报告 JSON'),
          ]),
          el('button.btn.btn-secondary.btn-sm', { onClick: () => this._exportCsv(res) }, [
            svgIcon('i-download'),
            document.createTextNode('逐点数据 CSV'),
          ]),
          el('button.btn.btn-secondary.btn-sm', { onClick: () => this._exportMetricsCsv(res) }, [
            svgIcon('i-download'),
            document.createTextNode('指标汇总 CSV'),
          ]),
        ]),
      ])
    );

 ...
 *   1. 使用层面：不同车型、不同摄像头位置需要微调阈值；
 *   2. 论文层面：做参数敏感性分析与消融实验时，改一个滑块就能出一组对照数据。
 */
...
/* 阈值滑块定义：[CONFIG 路径, 标签, min, max, step, 单位] */
const THRESHOLD_SPECS = [
  ['event.microsleepMs', '微睡眠判定时长', 300, 1200, 50, ' 毫秒'],
  ['event.criticalClosureMs', '危险闭眼时长', 1000, 4000, 100, ' 毫秒'],
  ['event.yawnMinMs', '哈欠最短持续', 600, 3000, 100, ' 毫秒'],
  ['event.nodPitchVelDegPerSec', '点头角速度阈值', 25, 120, 5, '°/秒'],
...
/* 窗口滑块定义 */
const WINDOW_SPECS = [
  ['window.perclosSec', 'PERCLOS 窗口', 15, 120, 5, ' 秒'],
  ['window.rateSec', '频率统计窗口', 20, 180, 10, ' 秒'],
  ['window.waveSec', '波形显示窗口', 10, 60, 5, ' 秒'],
  ['calibration.durationSec', '标定时长', 3, 20, 1, ' 秒'],
];

/* 演示模式标记（模块级状态），以 #swSimulate 的勾选状态为事实来源 */
let simulateOn = false;

/* 试听级别：[级别键, 按钮文案]，awake 无报警音不含 */
...
/* 同步滑块填充比例（供 CSS 渐变使用），让当前值在量程中的位置可见 */
...
/* 按 dotted path 读取 CONFIG 中的值 */
function getPath(obj, path) {
  return path.split('.').reduce((o, k) => (o ? o[k] : undefined), obj);
}

/* 按 dotted path 写入 CONFIG 中的值 */
...
export class SettingsPanel {
  ...
  show() {
    ...
    this.sheet.classList.add('was-open');
    this.backdrop.classList.add('open');
    this.backdrop.classList.add('was-open');
  ...
  /**
   * 移动端手势：向右滑动关闭面板。
   * 手指从面板左侧向右滑超过 80px 即关闭；松手后滑过阈值就关闭，否则弹回。
   */
  _bindSwipe() {
    let startX = 0;
    let currentX = 0;

 ...
 *
 * 双通道特征：
 *   几何通道：EAR（眼纵横比）、MAR（嘴纵横比）、头部欧拉角
 *   语义通道：eyeBlinkL/R、jawOpen、browDown（模型直接回归的表情系数）
 * 两通道互补融合：几何量在大角度侧脸时因投影压缩而失真，
 * 语义通道的 eyeBlink 系数仍然可靠；反之语义通道对轻微眯眼不敏感，由 EAR 补足。
 */
...
/**
 * 头部姿态。优先用模型输出的 4×4 面部变换矩阵，
 * 若该输出不可用，则退化为基于鼻-颏-额三点的几何近似。
 */
 ...
 * 关键点存在 1~2 像素固有抖动，直接求导（点头角速度）会被噪声淹没，
 * 因此对 EAR / MAR / 角度均施加短窗中值滤波，再计算差分。
 * 人脸丢失时滤波器与上一帧姿态一并复位，避免跨间断平滑污染。
 */
export class FeatureExtractor {
  ...
  /**
   * @param result MediaPipe FaceLandmarker 的 detectForVideo 返回值
   * @param ts     时间戳（毫秒，performance.now 基准）
   * @param aspect 画面宽高比（width / height），用于修正归一化坐标的各向异性
